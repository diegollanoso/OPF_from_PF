...
self.vg_inc = self.m.addMVar(data.n_gen_agc, vtype=self.gp.GRB.BINARY, name='vg_inc')
    ...
    def Flujos_TS(self ,data ,simm, ti, s, dpk, fp, fn, M = 100):
        ...
        # NO LOSSES
        #self.m.addConstr(f1-f2 <= (data.FMax[data.pos_ts]) * self.s_ts[:,s,ti], name = 'fs1_p') # 1
        #self.m.addConstr(f1-f2 >= -(data.FMax[data.pos_ts]) * self.s_ts[:,s,ti], name = 'fs1_n')

        #self.m.addConstr(fe+fv <= data.FMax[data.pos_nots], name = 'fs1_p') # 1
        #self.m.addConstr(fe+fv >= -data.FMax[data.pos_nots], name = 'fs1_p') # 1
