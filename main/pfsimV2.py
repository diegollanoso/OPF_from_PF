...
class PowerFactorySim(object):
    def __init__(self, project_name='Project', Agc: bool = True):
        self.Agc = Agc
        # Start PowerFactory
    ...
    def export_csv(self, flujo:str = 'AC'):
        ...
        export.f_name = r'C:\Users\lldie\OneDrive - Universidad Técnica Federico Santa María\Universidad\Memoria\Code\SF_datos_' + self.app.GetActiveProject().loc_name +'.csv'
        export.iopt_csel = 0 
        export.locn = 1
        export.iopt_sep=0
        export.iopt_locn = 1 
        ...
        ptdf_df = pd.read_csv(r'C:\Users\lldie\OneDrive - Universidad Técnica Federico Santa María\Universidad\Memoria\Code\SF_datos_' + self.app.GetActiveProject().loc_name +'.csv',skiprows = 0,delimiter=';')
    ...
    def get_data(self):
        ...
        for gen in self.dict_gen:
            ...
            if hasattr(self, 'Gen_AGC') and gen in self.Gen_AGC:
    ...
    def extract_data(self, elem: str, variable: str, return_time: bool = True):
...
class ShortSim(object):
    def __init__(self, data, big_optm, simm, new_SF, gen_out, ti):
        ...
        self.sum_gens = self.p_gen.sum() + self.p_genstat.sum() + self.pv_gen.sum()
        ...
        for line in data.app.GetCalcRelevantObjects('*.ElmLne'):
            if line.loc_name in new_SF.all_branch:
                perdidas = line.GetAttribute('c:Losses')/1000/data.Sb
                self.PL[new_SF.all_branch.index(line.loc_name)] = perdidas
                if perdidas == 0:
                    p_hv = line.GetAttribute('m:P:bushv')/data.Sb
                    p_lv = line.GetAttribute('m:P:buslv')/data.Sb
                    perdida = abs(p_hv - p_lv)
                    self.PL[new_SF.all_branch.index(line.loc_name)] = perdida
                    if perdida > 0:
                        print('Perdida en línea: ' + line.loc_name + ' = ' + str(perdida))
...
def Set_param_agc(pf, t_int, part_factors, previous_part_factors=None):
    ...
    for gen in pf.gen_agc:
        ...
        if (name_event not in (event.loc_name for event in pf.IntEvt.GetContents()) and 
            ...
            evento.mtime = t_int//60
                        #print('Evento en minuto: ' + str(t_int//60))
            if t_int//60 != 0:
                evento.time = t_int%60
                            #print('Evento en segundos: ' + str(t_int%60))
