...
for Case in CASES:
    ...
    m.addConstr(vg_dec.sum()==0, name = 'vg_dec=0')
    for ti in range(Nt):
        for s in range(Ns):
            ...
            m.addConstr(pg_inc[:,s,ti].sum() - pg_dec[:,s,ti].sum() + p_ens[:,s].sum() == P_out[s,ti] - dda_barra[:,ti].sum() + D_pfc[:,s,ti].sum() - PL_pre + ploss[:,s,ti].sum(), name ='Balance')
            ...
            m.addConstr(-f[:,s,ti] >= -FMax, name = 'FM+')
            m.addConstr(f[:,s,ti] >= -FMax, name = 'FM+')
    ...
    # %%
    with pd.ExcelWriter("0ResultadosOLD.xlsx") as writer:
    ...
    with pd.ExcelWriter('1ComparacionesOLD.xlsx') as writer:
        for ti in range(Nt):    
            for scen in range(Ns):
                delta_inc = np.zeros(ngen)
                delta_dec = np.zeros(ngen)
    ...
    with pd.ExcelWriter('3CasosOLD.xlsx') as writer:
        for ti in range(Nt):
            PL_pre = PL_pre_line[ti]/sim.Sb/1000 #+ PL_pre_trafo2[:,s].sum()
            P_agc=np.zeros(Ns)            
            costo = np.zeros(Ns)
            ...
            for scen in range(Ns):
                ...
                costo[scen] = Cvar_gen[pos_gen_agc_list]*sim.Sb @ (pg_inc[:,scen,ti].x)
            
            diff_power = np.array(pg_inc.x[:,:,ti]-pg_dec.x[:,:,ti])*sim.Sb
            diff_power = pd.DataFrame(np.vstack((Gen_outages,diff_power,P_agc*sim.Sb,P_out[:,ti]*sim.Sb,costo)).T, columns = [escenariosPy_col[ti]] + name_gen_agc_list + ['P_agc', 'P_out', 'Costo'])
