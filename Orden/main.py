...
# demanda por barra
dda_barra = np.zeros((len(indices_bus), Nt))
...
D_pfc = np.zeros((len(indices_bus), Ns, Nt)) # Demanda al finaliza el CPF
PL_pre_line = np.zeros((Ns,Nt)) # Perdidas al finalizar el CPF
...
for ti in range(Nt):
    ...
    for gen_out in Gen_outages:
        print('Progreso: ' + str(cont+1+Ns*ti) + '/' + str(Ns*Nt))
        print('Generador Out: ' + gen_out)
        P_out[cont] = dict_gen[gen_out][8+ti]/sim.Sb
        ...
        if PL_pre_line[cont,ti] == 0: # Obtener perdidas solo una vez para cada contingencia y cada operación demanda
            PL_pre_line[cont,ti] = np.array(list(map(lambda x: x.GetAttribute('c:Losses'), sim.lineas))).sum()
...
p_ens = m.addMVar((len(indices_bus),Ns, Nt), vtype=GRB.CONTINUOUS, ub=GRB.INFINITY, lb=0, name='P_ens')
...
for ti in range(Nt):
    ...
    Cpl = 0
    for s in range(Ns):
        Cop += Gen_on_AGC[ti,:,s][pos_gen_agc_list] * Cvar_gen[pos_gen_agc_list]*sim.Sb @ (pg_inc[:,s,ti] + pg_dec[:,s,ti])
        if np.size(Cvar_genstat) != 0:
            Cop += Genstat_on_AGC[:,s] * Cvar_genstat*sim.Sb @ pstatg_inc[:,s]

        Cue += (VOLL * p_ens[:,s,ti]).sum() * sim.Sb
        Cpl += (VOLL * ploss[:,s,ti]).sum() * sim.Sb

    f_obj_1 += (Cop + Cue + Cpl)/Ns
...
for ti in range(Nt):
    ...
    for s in range(Ns):
        PL_pre = PL_pre_line[s,ti].sum()/sim.Sb/1000 #+ PL_pre_trafo2[:,s].sum()
        #Balance
        ...
        m.addConstr(pg_inc[:,s,ti].sum() - pg_dec[:,s,ti].sum() + pstatg_inc[:,s].sum() - pstatg_dec[:,s].sum() + p_ens[:,s].sum() == P_out[s] - dda_barra[:,ti].sum() + D_pfc[:,s,ti].sum() - PL_pre + ploss[:,s,ti].sum(), name ='Balance')
        #Flujo2
        f_ens =  SF @ p_ens[:,s,ti]
        ...
        if Gen_outages[s] in name_gen_agc_list:
            ...
            m.addConstr(pg_inc[Gen_AGC.index(Gen_outages[s]), s, ti] == 0, name = 'GenOut+')
            m.addConstr(pg_dec[Gen_AGC.index(Gen_outages[s]), s, ti] == 0, name = 'GenOut-')
...
if status == GRB.Status.OPTIMAL:
    ...
    for gen, s, ti in np.ndindex(pg_inc.x.shape):
        Post_gen_out[s, ti] = P_out[s] - dda_barra[:,ti].sum() + D_pfc[:,s,ti].sum() - PL_pre_line[s,ti].sum()/sim.Sb/1000 +  ploss[:,s,ti].x.sum()
...
#2  
dddf = np.vstack((range(1,Ns*Nt+1),np.tile(Gen_outages,Nt),np.tile(P_out,Nt),np.tile(Barra_gen_out,Nt)))
...
for ti in range(Nt):
    ...
    for scen in range(Ns):
        ...
        for i_gen in range(len(Gen_AGC)):
            ...
            evt2.outserv = 0
            evt2.value = str(part_factors[:,scen,ti][i_gen])
        sim.prepare_dynamic_sim({}, 'rms', end_time=t_final)
...
with pd.ExcelWriter('1Comparaciones.xlsx') as writer:
    for scen in range(Ns):
        ...
        name_hoja = 'E ' + str(scen) + ' ' + Gen_outages[scen]
