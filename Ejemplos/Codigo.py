...
# Importar DIgSILENT
import sys
sys.path.append(r'C:\Program Files\DIgSILENT\PowerFactory 2020 SP1\Python\3.8')
import powerfactory
...
factores = app.GetFromStudyCase('ComVstab') # PTDF
...
for i in range(nl):
    ...
    model.addConstr( float(fm[i]) >= f[i],name ='f_max')
    model.addConstr( float(fm[i]) >= -f[i],name ='f_max')
...
# Balance de potencia 
model.addConstr(p.sum('*') == demanda_total*fact_perd, name ='Balance')    
